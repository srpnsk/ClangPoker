...
/*
 * крч, ввожу в курс дела
 * мы делаем сервера, который принимает в себя клиентов
 * изначально у клиента состояние UNKNOWN
 * спрашиваем у него имя, и мы его переводим в состояние IN_LOBBY
 * в лобби у него спрашиваем его пожелания по комнате и тогда мы отправляем в
 * комнату
 *
 * затем сервер создает ребенка
 * и этот ребенок управляет комнатой
 * нет, в сервере.ц мы прописали логику общего сервера, а в рум.ц -- логику
 * комнатки
 * */
...
int create_room_process(int room_id) {
  int socket_pair[2]; // и вот начинается адовая дрочь
                      // крч, мы создаем мать его СОКЕТ для связи между основным
                      // сервером (родителем) и ребенком крч, аналог 1 и 2
                      // задания на полусем, но через мать его СОКЕТЫ
